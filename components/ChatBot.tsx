...
/** Small sparkle mark used as the assistant's avatar — the site's brand mark. */
...
/** The circle + triangle "See it in Action" mark used elsewhere on the site, reused here for consistency. */
function PlayMark() {
  return (
    <span className="relative flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-ink/25">
    ...
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
        ...
        <div className="flex h-[32rem] w-[21rem] flex-col overflow-hidden rounded-[22px] border border-ink-line bg-ink shadow-2xl shadow-black/60 sm:w-[23rem]">
          ...
          <div className="flex items-center justify-between border-b border-ink-line bg-ink-soft/60 px-4 py-3.5">
            <div className="flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-signal-gradient text-ink shadow-[0_0_18px_-2px_rgba(255,106,0,0.6)]">
                <SparkMark className="h-4.5 w-4.5" />
              </span>
              <div>
      ...
      >
        ...
        <span className="focus-ring relative flex h-14 w-14 items-center justify-center rounded-full bg-signal-gradient text-ink shadow-lg shadow-black/40 transition-transform group-hover:scale-105">
          {open ? <span className="text-2xl leading-none">&times;</span> : <SparkMark className="h-6 w-6" />}
        </span>
      </button>
    </div>
  );
